...
url_1 = "https://www.volby.cz/pls/ps2017nss/ps32?xjazyk=CZ&xkraj=4&xnumnuts=3202"  # KLATOVY
#url_1 = "https://www.volby.cz/pls/ps2017nss/ps32?xjazyk=CZ&xkraj=11&xnumnuts=6204" # BŘECLAV  
#url_1 = "https://www.volby.cz/pls/ps2017nss/ps32?xjazyk=CZ&xkraj=12&xnumnuts=7101"  # JESENÍK        
...
def scraping_of_election_results(tr_tags: "bs4.element.ResultSet") -> list:
    ...
    for tr in tr_tags[2:]:
        ...
        #print("td on the row:", td_on_the_row)
        ...
        RESULTS = iteration_of_results_1(tr_tags, RESULTS_1)                                                                 
...
def generation_of_url_each_municipality(tr:"bs4.element.Tag") -> str:
    ...
    if len(url[9:]) == 72:
        url_election_link = "https://www.volby.cz/pls/ps2017nss/" + url[9:69].replace("&amp;", "&")
    elif len(url[9:]) == 73:
        url_election_link = "https://www.volby.cz/pls/ps2017nss/" + url[9:70].replace("&amp;", "&")       
...
def iteration_of_results_1(tr_tags: "bs4.element.ResultSet", RESULTS_1: list) -> list:
    """
    ITERATION OF RESULTS_1, append to RESULTS,
    tr_tags[-2].find("td", {"class":"hidden_td"}) - finding of empty rows under the last municipality,
    len(RESULTS_1) == len(tr_tags[2:]) - 2 represents the number of municipalities
    return RESULTS - final results, list with dictionaries that contains code, location and the election returns
    """
    RESULTS = list()

    if tr_tags[-2].find("td", {"class":"hidden_td"}):                   
        for result in RESULTS_1:        
            if len(RESULTS_1) == len(tr_tags[2:]) - 2:                  
                RESULTS.append(result)
    elif tr_tags[-1].find("td", {"class":"hidden_td"}):
        for result in RESULTS_1:
            if len(RESULTS_1) == len(tr_tags[2:]) - 1:
                RESULTS.append(result)
    else:
        for result in RESULTS_1:
            if len(RESULTS_1) == len(tr_tags[2:]):
                RESULTS.append(result) 
